...
async def attach_deposit_receipt(deposit_id: int, file_id: str):
    ...
    async with _pool.acquire() as conn:
        await conn.execute(
            "UPDATE deposits SET receipt_file_id=$1 WHERE id=$2",
            file_id, deposit_id
        )
    ...
    async with _pool.acquire() as conn:
        async with conn.transaction():
            ...
            user_balance = await conn.fetchrow(
                "SELECT g_balance, total_spent FROM users WHERE user_id=$1 FOR UPDATE",
                user_id
            )
            if not user_balance:
                return False
            ...
            new_g_balance = float(user_balance["g_balance"]) + float(dep["amount_gt"])
            new_total_spent = float(user_balance["total_spent"]) + float(dep["amount_tenge"])
            await conn.execute(
                "UPDATE users SET g_balance=$1, total_spent=$2 WHERE user_id=$3",
                new_g_balance, new_total_spent, user_id
            )
            await conn.execute(
...
async def reject_deposit(deposit_id: int, admin_id: int, reason: Optional[str] = None) -> bool:
    global _pool
    ...
    async with _pool.acquire() as conn:
        ...
        await conn.execute(
            "INSERT INTO admin_notifications (type, entity_id) VALUES ('deposit_rejected',$1)",
            deposit_id
        )
    return True
...
async def attach_withdraw_screenshot(withdraw_id: int, file_id: str):
    ...
    async with _pool.acquire() as conn:
        await conn.execute(
            "UPDATE withdrawals SET screenshot_file_id=$1 WHERE id=$2",
            file_id, withdraw_id
        )
    ...
    async with _pool.acquire() as conn:
        async with conn.transaction():
            ...
            balance = await conn.fetchval(
                "SELECT g_balance FROM users WHERE user_id=$1 FOR UPDATE",
                user_id
            )
            ...
            await conn.execute(
                "UPDATE users SET g_balance=g_balance-$1 WHERE user_id=$2",
                float(w["amount_g"]), user_id
            )
            await conn.execute(
...
async def reject_withdrawal(withdraw_id: int, admin_id: int, reason: Optional[str] = None) -> bool:
    global _pool
    note = f"Rejected by {admin_id}: {reason or ''}"
    async with _pool.acquire() as conn:
        await conn.execute(
